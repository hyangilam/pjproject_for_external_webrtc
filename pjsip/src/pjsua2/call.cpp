...
void CallInfo::fromPj(const pjsua_call_info &pci)
{
    ...
    remoteDisplay       = pj2Str(pci.remote_info_Display);
    remoteUser          = pj2Str(pci.remote_info_User);
    remoteContact       = pj2Str(pci.remote_contact);
    remoteIsFocus       = pci.remote_isfocus;
    callIdString        = pj2Str(pci.call_id);
	toHeaderString      = pj2Str(pci.toheader);
    sktnumpidHeaderString      = pj2Str(pci.sktnumpidheader);
    tphoneString = pj2Str(pci.tphone);
    reasonString        = pj2Str(pci.reason);
	xTypeString			= pj2Str(pci.xtype);
    pAssertedIDString	= pj2Str(pci.passertedid);
    paidDisplay         = pj2Str(pci.paidDisplay);
    paidUser            = pj2Str(pci.paidUser);
	xInfoString			= pj2Str(pci.xinfo);
	xDidString          = pj2Str(pci.xdid);
